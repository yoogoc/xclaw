...
impl<M: CompletionModel> Binding<M> {
    // agent core loop
    pub async fn run_loop(&self, session: Arc<Mutex<Session>>, thread_id: Uuid) -> anyhow::Result<()> {
        loop {
            let outcome = self.run_agentic_loop(session.clone(), thread_id).await?;

            match outcome {
                LoopOutcome::Response(response) => {
                    self.session_manager.thread_complete_turn(session.clone(), thread_id, response.content).await;
                    break;
                }
                ...
                LoopOutcome::ToolCall { approvals, .. } => {
                    ...
                    if all_auto {
                        ...
                        if let Some(thread) = sess.threads.get_mut(&thread_id) {
                            ...
                            let pa_json = serde_json::to_string(&thread.pending_approvals).unwrap_or_else(|_| "[]".to_string());

                            self.session_manager
                                .persist_thread_awaiting_approval(&thread.id.to_string(), &pa_json, &thread.updated_at.to_rfc3339())
                                .await;
                        }
                        break;
                    }
                ...
                LoopOutcome::MaxIterations => {
                    self.session_manager.thread_fail_turn(session.clone(), thread_id, "Max iterations reached").await;
                    break;
                }
    ...
    // agent core loop
    pub async fn run_agentic_loop(&self, session: Arc<Mutex<Session>>, thread_id: Uuid) -> anyhow::Result<LoopOutcome> {
        let current_iteration = {
            let sess = session.lock().await;
            sess.threads.get(&thread_id).and_then(|t| t.last_turn()).map(|turn| turn.current_tool_iterations).unwrap_or(0)
        };
                        ...
                        if let Some(turn) = thread.last_turn_mut() {
                            ...
                            match result {
                                ...
                                Err(e) => {
                                    ...
                                    drop(sess);
                                    self.session_manager.persist_tool_result(&turn_id, idx, None, Some(&err_str)).await;
                                }
                            }
                        }
                    }
