...
impl <C: Channel>ChannelManager<C> {
    ...
    pub async fn chunk_send(&self, msg: &IncomingMessage, response: OutgoingResponse) -> anyhow::Result<MessageStream> {
        // 兼容draft edits：
    ...
    pub async fn flush(&self, msg: &IncomingMessage, response: OutgoingResponse) -> anyhow::Result<MessageStream> {
        // 兼容draft edits：
        // 如果channel支持draft edits且配置了流式发送，那么就直接发送，
        // 如果不支持draft edits
        todo!()
